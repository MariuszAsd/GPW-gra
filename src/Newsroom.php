...
final class Newsroom
{
    ...
    /**
     * Szablony spółkowe: kod => [kind, type, espi, impact, dur, weight, head, body].
    ...
    public const COMPANY = [
        ...
        'kontrakt_eksport' => ['fundamental', 'POS', 1, 0.55, 12, 9,
            ...
            'Umowa odpowiada ok. {pctrev}% rocznych przychodów {name} i będzie realizowana przez najbliższe miesiące. Zarząd deklaruje, że rentowność kontraktu jest zbliżona do średniej marży grupy, więc podpis powinien niemal wprost przełożyć się na wynik netto.'],
        'umowa_ramowa' => ['fundamental', 'POS', 1, 0.35, 12, 7,
            'ESPI: {ticker} zawiera umowę ramową z {klient} — do {mln} mln PLN w 3 lata',
            'Umowa ramowa nie gwarantuje całej kwoty, ale otwiera drzwi do regularnych zamówień. Pierwsze zlecenie za {mln2} mln PLN trafi do realizacji jeszcze w tym kwartale — analitycy lubią takie kontrakty za przewidywalność przychodów.'],
        'buyback' => ['fundamental', 'POS', 1, 0.4, 14, 6,
            'ESPI: {ticker} ogłasza skup akcji własnych do {pct2}% kapitału',
            'Skup finansowany z nadwyżek gotówki zmniejszy liczbę akcji w obrocie — ten sam zysk rozłoży się na mniej walorów, więc zysk na akcję (EPS) mechanicznie wzrośnie. Rynek odbiera buyback także jako sygnał, że zarząd uważa kurs za zbyt niski.'],
        'prognoza_gora' => ['fundamental', 'POS', 1, 0.5, 12, 6,
            'ESPI: zarząd {ticker} podnosi prognozę rocznego zysku o {pct}%',
            'Powodem jest lepszy od planu portfel zamówień i niższe koszty finansowania. Podniesiona prognoza to jedna z najsilniejszych informacji fundamentalnych — zarząd rzadko ryzykuje własną wiarygodność bez pokrycia w liczbach.'],
        'duzy_klient' => ['fundamental', 'POS', 1, 0.4, 10, 7,
            '{ticker} pozyskuje strategicznego klienta — {klient}',
            'Nowy klient ma docelowo odpowiadać za kilka procent przychodów {name}. Dywersyfikacja bazy klientów obniża ryzyko biznesu: spółka będzie mniej zależna od pojedynczych kontraktów.'],
        'patent_wdrozenie' => ['fundamental', 'POS', 1, 0.35, 12, 5,
            '{ticker} wdraża własną technologię — koszty jednostkowe spadną o ok. {pct2}%',
            'Po dwóch latach testów spółka uruchamia rozwiązanie chronione patentem. Niższy koszt wytworzenia przy tych samych cenach sprzedaży to wprost wyższa marża — efekty mają być widoczne w wynikach już od przyszłego kwartału.'],
        'dotacja_ue' => ['fundamental', 'POS', 1, 0.25, 10, 5,
            '{ticker} z grantem {mln2} mln PLN na prace badawczo-rozwojowe',
            'Zewnętrzne finansowanie pokryje większość budżetu R&D na najbliższy rok, odciążając własną gotówkę spółki. Projekt dotyczy rozwoju nowej linii produktowej dla sektora {sector}.'],
        'sprzedaz_aktywow' => ['fundamental', 'POS', 1, 0.3, 10, 4,
            'ESPI: {ticker} sprzedaje niestrategiczne aktywa za {mln2} mln PLN',
            'Transakcja z zyskiem księgowym jednorazowo podbije najbliższy wynik, a uwolniona gotówka ma sfinansować rozwój podstawowego biznesu. Uwaga inwestorów: to zysk JEDNORAZOWY — nie poprawia trwałej rentowności.'],
        'rating_gora' => ['fundamental', 'POS', 0, 0.3, 10, 4,
            'Agencja podnosi ocenę wiarygodności kredytowej {ticker}',
            'Lepszy rating to tańszy dług: przy refinansowaniu obligacji spółka zaoszczędzi na odsetkach. Agencja doceniła spadek zadłużenia i stabilność przepływów operacyjnych {name}.'],
        'moce_produkcyjne' => ['fundamental', 'POS', 1, 0.3, 12, 5,
            '{ticker} uruchamia nową linię — moce produkcyjne w górę o {pct}%',
            'Inwestycja pozwoli obsłużyć rosnący portfel zamówień bez kosztownych nadgodzin i podwykonawców. Pełne wykorzystanie nowych mocy planowane jest w ciągu dwóch–trzech kwartałów.'],
        'wejscie_kraj' => ['fundamental', 'POS', 1, 0.3, 12, 6,
            '{ticker} otwiera sprzedaż na rynku: {kraj}',
            'Ekspansja zagraniczna zwiększa adresowalny rynek {name}, choć na starcie dołoży kosztów marketingu i logistyki. Zarząd celuje w rentowność nowego rynku po czterech kwartałach.'],
        'rekord_operacyjny' => ['fundamental', 'POS', 0, 0.3, 8, 6,
            'Dane operacyjne {ticker}: rekordowa miesięczna sprzedaż',
            'Opublikowane dane sprzedażowe wyprzedzają raport finansowy i sugerują, że nadchodzące wyniki mogą pozytywnie zaskoczyć. To tzw. twardy sygnał wyprzedzający — rynek zwykle nie czeka z reakcją do dnia raportu.'],
        'redukcja_dlugu' => ['fundamental', 'POS', 1, 0.25, 10, 4,
            'ESPI: {ticker} spłaca obligacje przed terminem',
            'Przedterminowy wykup obniży koszty odsetkowe w kolejnych kwartałach i poprawi strukturę bilansu. Spółka finansuje operację z bieżących przepływów — bez nowej emisji.'],
        'przejecie_male' => ['fundamental', 'POS', 1, 0.4, 12, 4,
            'ESPI: {ticker} przejmuje mniejszego konkurenta za {mln2} mln PLN',
            'Przejęcie powiększy udziały rynkowe i portfel klientów {name}; synergie kosztowe mają pokryć cenę zakupu w ok. trzy lata. Ryzykiem pozostaje integracja zespołów — historia zna fuzje, które zjadły więcej, niż obiecywały.'],

        // ===== FUNDAMENTY — negatywne =====
        'utrata_klienta' => ['fundamental', 'NEG', 1, -0.5, 12, 6,
            'ESPI: kluczowy klient {ticker} nie przedłuża umowy',
            'Wygasający kontrakt odpowiadał za ok. {pctrev}% przychodów {name}. Zarząd zapewnia, że trwają rozmowy z nowymi odbiorcami, ale luki w przychodach nie da się wypełnić z miesiąca na miesiąc.'],
        'kara_regulatora' => ['fundamental', 'NEG', 1, -0.45, 12, 5,
            'Regulator nakłada na {ticker} karę {mln2} mln PLN',
            'Kara obciąży wynik najbliższego kwartału. Spółka zapowiada odwołanie, ale ostrożnościowo zawiąże rezerwę na całą kwotę — tak każe sztuka księgowa. Gorsze od samej kary bywa ryzyko zaostrzenia nadzoru.'],
        'odpis_aktywow' => ['fundamental', 'NEG', 1, -0.5, 12, 4,
            'ESPI: {ticker} dokonuje odpisu wartości aktywów na {mln} mln PLN',
            'Odpis jest bezgotówkowy — pieniądze nie wypływają ze spółki — ale pomniejsza wynik netto i kapitały własne. To księgowe przyznanie, że część wcześniejszych inwestycji nie zarobi tyle, ile zakładano.'],
        'prognoza_dol' => ['fundamental', 'NEG', 1, -0.55, 12, 5,
            'ESPI: zarząd {ticker} obniża prognozę wyników o {pct}%',
            'Powody: słabszy popyt i rosnące koszty. Obniżka prognozy to poważny sygnał — zarządy robią to niechętnie i zwykle dopiero wtedy, gdy problemy są już nie do zamaskowania.'],
        'presja_kosztow' => ['fundamental', 'NEG', 0, -0.3, 10, 6,
            '{ticker} pod presją kosztów — marże się kurczą',
            'Drożejąca energia i wynagrodzenia zjadają rentowność {name}, a konkurencja nie pozwala w pełni przerzucić kosztów na ceny. Kluczowe pytanie przed najbliższym raportem: ile z tej presji spółka zdołała zamortyzować.'],
        'audyt_zastrzezenia' => ['fundamental', 'NEG', 1, -0.6, 14, 3,
            'Biegły rewident zgłasza zastrzeżenia do sprawozdania {ticker}',
            'Audytor kwestionuje sposób ujęcia części przychodów. To nie wyrok — ale rynek traktuje zastrzeżenia audytu jak żółtą kartkę dla wiarygodności liczb, na których opiera się cała wycena.'],
        'rezerwa_naleznosci' => ['fundamental', 'NEG', 1, -0.35, 10, 4,
            'ESPI: kontrahent {ticker} przestał płacić — rezerwa {mln2} mln PLN',
            'Spółka zawiąże rezerwę na nieściągalne należności, co obciąży najbliższy wynik. Zarząd kieruje sprawę do sądu, ale odzyskiwanie pieniędzy z upadłego kontrahenta potrafi trwać latami.'],
        'rating_dol' => ['fundamental', 'NEG', 0, -0.35, 10, 4,
            'Agencja obniża ocenę wiarygodności kredytowej {ticker}',
            'Niższy rating oznacza droższe finansowanie przy najbliższym rolowaniu długu. Agencja wskazuje na rosnące zadłużenie i pogarszającą się płynność sektora {sector}.'],
        'awaria_powazna' => ['fundamental', 'NEG', 1, -0.35, 10, 4,
            'ESPI: poważna awaria w zakładzie {ticker} — przestój do dwóch tygodni',
            'Utracona produkcja i koszty napraw obciążą bieżący kwartał; część zamówień przejmie konkurencja. Ubezpieczenie pokryje szkody majątkowe, ale nie utracone przychody.'],
        'recall_produktu' => ['fundamental', 'NEG', 1, -0.4, 12, 3,
            '{ticker} wycofuje partię produktu z rynku',
            'Wykryta wada oznacza koszty serwisu, logistyki zwrotów i — trudniejszą do wyceny — rysę na zaufaniu klientów. Skala finansowa będzie znana za kilka tygodni; niepewność sama w sobie ciąży kursowi.'],
        'przegrany_proces' => ['fundamental', 'NEG', 1, -0.4, 10, 3,
            'Sąd: {ticker} zapłaci {mln2} mln PLN odszkodowania',
            'Wyrok pierwszej instancji w wieloletnim sporze. Spółka zapowiada apelację, lecz zgodnie z zasadą ostrożności rezerwa obciąży wynik już teraz.'],
        'emisja_akcji' => ['fundamental', 'NEG', 1, -0.45, 12, 3,
            'ESPI: {ticker} planuje emisję akcji — rozwodnienie ok. {pct2}%',
            'Nowe akcje sfinansują inwestycje, ale dotychczasowi akcjonariusze będą mieli mniejszy udział w tym samym zysku — EPS spadnie, zanim inwestycje zaczną zarabiać. Rynek zwykle wycenia rozwodnienie od razu.'],
        ...
        'wywiad_prezes' => ['sentiment', 'POS', 0, 0.3, 8, 7,
            ...
            '{osoba}, prezes {name}, zapowiada w mediach dwucyfrowy wzrost i „projekty, o których rynek jeszcze nie wie". Deklaracje bez liczb to paliwo dla nastrojów — fundamenty poznamy dopiero w raporcie.'],
        'fundusz_wchodzi' => ['sentiment', 'POS', 1, 0.35, 10, 5,
            'ESPI: {fundusz} ujawnia pakiet powyżej 5% akcji {ticker}',
            'Wejście dużego instytucjonalnego gracza rynek czyta jako wotum zaufania — fundusze rzadko kupują bez własnej analizy. Sceptycy przypomną: fundusze też się mylą, a duży pakiet kiedyś trzeba będzie sprzedać.'],
        'blog_hype' => ['sentiment', 'POS', 0, 0.25, 6, 6,
            'Popularny analityk chwali model biznesowy {ticker}',
            'Wpis z szeroko udostępnianą analizą przyciąga uwagę inwestorów indywidualnych. Uwaga: to opinia, nie rekomendacja domu maklerskiego — bez nowych liczb ze spółki entuzjazm bywa krótkotrwały.'],
        'ranking_pracodawcow' => ['sentiment', 'POS', 0, 0.15, 6, 4,
            '{ticker} wysoko w rankingu najlepszych pracodawców',
            'Łatwiejsza rekrutacja i mniejsza rotacja to realna, choć trudno mierzalna przewaga. Bezpośredniego wpływu na najbliższe wyniki brak — ale wizerunek pracuje długo.'],
        'spekulacja_dywidenda' => ['sentiment', 'POS', 0, 0.3, 8, 4,
            'Rynek spekuluje: {ticker} może podnieść dywidendę',
            'Po serii dobrych przepływów gotówkowych część inwestorów obstawia hojniejszą wypłatę. Zarząd milczy — a spekulacja pozostaje spekulacją, dopóki nie potwierdzi jej uchwała.'],
        'forum_hype' => ['sentiment', 'POS', 0, 0.3, 6, 5,
            'Inwestorzy indywidualni rozgrzali forum na temat {ticker}',
            'Liczba wzmianek o spółce rośnie lawinowo, a wraz z nią drobne zlecenia kupna. Tłum potrafi podbić kurs szybciej niż fundamenty — i równie szybko się znudzić.'],
        'nagroda_branzowa' => ['sentiment', 'POS', 0, 0.15, 6, 4,
            '{name} z nagrodą branżową za innowację roku',
            'Prestiż i darmowy marketing w mediach {sector}. Na wyniki finansowe nagroda nie działa wprost, ale wzmacnia pozycję w rozmowach z klientami.'],
        'sponsoring_glosny' => ['sentiment', 'POS', 0, 0.2, 6, 3,
            '{ticker} zostaje sponsorem głośnego wydarzenia sportowego',
            'Rozpoznawalność marki w górę, budżet marketingowy w dół. Czy przełoży się to na sprzedaż — zobaczymy w danych za kilka miesięcy; na razie kupują głównie nagłówki.'],

        // ===== NASTROJE — negatywne =====
        'krytyka_mediow' => ['sentiment', 'NEG', 0, -0.3, 8, 5,
            'Reportaż o warunkach pracy w {ticker} wywołuje burzę',
            'Materiał zdobywa zasięgi, a spółka publikuje oświadczenie. Bezpośrednich skutków finansowych na razie brak — ale presja mediów bywa przedsionkiem kontroli i pozwów.'],
        'fundusz_wychodzi' => ['sentiment', 'NEG', 1, -0.35, 10, 4,
            'ESPI: {fundusz} redukuje zaangażowanie w {ticker} poniżej progu 5%',
            'Wyjście instytucji zawsze rodzi pytanie: co takiego zobaczyli w spółce? Czasem odpowiedź jest prozaiczna — przebudowa portfela — ale podaż dużego pakietu i tak ciąży notowaniom.'],
        'plotka_kadrowa' => ['sentiment', 'NEG', 0, -0.25, 8, 4,
            'Nieoficjalnie: dyrektor finansowy {ticker} rozważa odejście',
            'Plotka kadrowa obniża komfort inwestorów — CFO zna liczby spółki najlepiej. Bez oficjalnego komunikatu to tylko szum, lecz rynek woli dmuchać na zimne.'],
        'forum_panika' => ['sentiment', 'NEG', 0, -0.35, 6, 4,
            'Wpis „sygnalisty" o {ticker} podgrzewa panikę wśród drobnych inwestorów',
            'Anonimowe oskarżenia rozchodzą się szybciej niż sprostowania. Spółka zaprzecza, dowodów brak — ale część detalicznych graczy woli najpierw sprzedać, potem pytać.'],
        'butik_przewartosciowanie' => ['sentiment', 'NEG', 0, -0.3, 8, 4,
            'Butik analityczny: wycena {ticker} „oderwana od fundamentów"',
            'Raport niezależnych analityków wskazuje na zbyt wysoki wskaźnik C/Z na tle branży. To opinia, nie fakt — ale daje amunicję tym, którzy czekali na pretekst do realizacji zysków.'],
        'bojkot_social' => ['sentiment', 'NEG', 0, -0.25, 6, 3,
            'W sieci narasta akcja bojkotu produktów {ticker}',
            'Hasztagi rosną, choć historia uczy, że internetowe bojkoty rzadko widać w twardych danych sprzedaży. Ryzyko rośnie, jeśli podchwycą je media głównego nurtu.'],
        'spor_akcjonariuszy' => ['sentiment', 'NEG', 0, -0.3, 10, 3,
            'Konflikt w akcjonariacie {ticker} o strategię spółki',
            'Dwie grupy akcjonariuszy forsują sprzeczne wizje rozwoju. Spory właścicielskie paraliżują decyzje zarządu i potrafią wisieć nad kursem miesiącami.'],
        'zwiazki_spor' => ['sentiment', 'NEG', 0, -0.25, 8, 3,
            'Związki zawodowe w {ticker} zapowiadają spór zbiorowy',
            'Żądania płacowe na razie bez strajku, ale napięcie rośnie. Jeśli dojdzie do przestojów, koszty z miękkich zrobią się twarde.'],
    ];
    ...
    public const SECTOR = [
        'eksport_branzy' => ['fundamental', 'POS', 0, 0.3, 12, 6,
            ...
            'Dane celne pokazują rosnący popyt zagraniczny na produkty sektora. Więcej zamówień u wszystkich graczy — od liderów po maruderów.'],
        'popyt_krajowy_neg' => ['fundamental', 'NEG', 0, -0.3, 12, 6,
            'Popyt krajowy w branży {sector} hamuje',
            'Konsumenci i firmy odkładają zakupy; sektor wchodzi w okres walki o klienta cenami, co zwykle kończy się erozją marż w całej branży.'],
        'surowce_tanieja' => ['fundamental', 'POS', 0, 0.35, 12, 5,
            'Kluczowe surowce dla sektora {sector} tanieją na rynkach światowych',
            'Niższe ceny wsadu to wyższe marże wytwórców — efekt widać w wynikach zwykle po jednym, dwóch kwartałach, gdy skończą się stare, droższe zapasy.'],
        'surowce_drozeja' => ['fundamental', 'NEG', 0, -0.35, 12, 5,
            'Skokowy wzrost cen surowców uderza w sektor {sector}',
            'Koszty produkcji rosną szybciej, niż spółki są w stanie podnosić ceny. Przewagę zyskują firmy z długimi kontraktami na dostawy — reszta zapłaci rynkową stawkę.'],
        'zmiana_podatkowa' => ['fundamental', 'NEG', 0, -0.3, 14, 4,
            'Projekt zmian podatkowych obciąży branżę {sector}',
            'Nowa danina sektorowa w konsultacjach. Do uchwalenia daleko, ale rynek dyskontuje ryzyko od razu — wyceny spółek z branży pod presją.'],
        'dane_branzy_pos' => ['fundamental', 'POS', 0, 0.25, 10, 6,
            'Raport branżowy: sprzedaż w sektorze {sector} powyżej oczekiwań',
            'Zagregowane dane sprzedażowe branży wyprzedzają raporty pojedynczych spółek — statystycznie dobre otoczenie podnosi prognozy dla wszystkich graczy sektora.'],
        'dane_branzy_neg' => ['fundamental', 'NEG', 0, -0.25, 10, 6,
            'Raport branżowy: sektor {sector} wytraca tempo',
            'Wolumeny sprzedaży niższe rok do roku. Analitycy zaczną ścinać prognozy dla spółek z branży — pytanie tylko, kto ucierpi najmniej.'],
        'inwestor_zagraniczny' => ['fundamental', 'POS', 0, 0.3, 12, 4,
            'Globalny gracz zapowiada inwestycje w polski sektor {sector}',
            'Kapitał zagraniczny podnosi wyceny w całej branży: rośnie szansa na przejęcia lokalnych firm z premią do kursu.'],
        'konferencja_optymizm' => ['sentiment', 'POS', 0, 0.2, 8, 5,
            'Optymizm na konferencji branżowej sektora {sector}',
            'Prezesi prześcigają się w dobrych zapowiedziach, a kuluary huczą od planów ekspansji. Nastroje to nie zamówienia — ale rynek lubi dobrą atmosferę.'],
        'medialna_moda' => ['sentiment', 'POS', 0, 0.3, 8, 4,
            'Media okrzyknęły {sector} „branżą przyszłości"',
            'Seria entuzjastycznych publikacji ściąga kapitał detaliczny do całego sektora. Doświadczeni gracze wiedzą: gdy branża trafia na okładki, do rozdania zostaje zwykle mniej, niż się wydaje.'],
        'medialna_niechec' => ['sentiment', 'NEG', 0, -0.3, 8, 4,
            'Sektor {sector} traci błysk w oczach inwestorów',
            'Moda się skończyła: media piszą o „przegrzanych wycenach", a kapitał rotuje do innych branż. Fundamentów to nie zmienia — ale przepływy robią kursy.'],
        'niedobor_kadr_sektor' => ['fundamental', 'NEG', 0, -0.25, 12, 4,
            'Branża {sector} zmaga się z niedoborem specjalistów',
            'Presja płacowa podnosi koszty w całym sektorze; firmy konkurują o tych samych ludzi. Najbardziej ucierpią spółki z niską marżą, które nie mają z czego dokładać do pensji.'],
    ];
    ...
    public const MARKET = [
        'makro_konsument_pos' => ['fundamental', 'POS', 0, 0.15, 10, 6,
            ...
            'Konsumenci wydają śmielej, niż zakładali ekonomiści — dobry omen dla spółek handlowych i dóbr konsumenckich, pośrednio dla całego rynku.'],
        'makro_konsument_neg' => ['fundamental', 'NEG', 0, -0.15, 10, 6,
            'Dane makro: sprzedaż detaliczna rozczarowuje',
            'Słabszy popyt konsumencki studzi oczekiwania wobec wyników spółek na najbliższe kwartały.'],
        'makro_produkcja_pos' => ['fundamental', 'POS', 0, 0.15, 10, 5,
            'Produkcja przemysłowa zaskakuje na plus',
            'Fabryki zwiększają wykorzystanie mocy — sygnał ożywienia, który zwykle wyprzedza poprawę zysków w przemyśle i transporcie.'],
        'makro_produkcja_neg' => ['fundamental', 'NEG', 0, -0.15, 10, 5,
            'Produkcja przemysłowa poniżej oczekiwań',
            'Zamówienia w przemyśle słabną. Jeśli trend się utrzyma, analitycy zaczną obniżać prognozy dla spółek cyklicznych.'],
        'naplywy_tfi' => ['sentiment', 'POS', 0, 0.15, 8, 4,
            'Fundusze akcji notują najwyższe napływy od miesięcy',
            'Świeży kapitał od oszczędzających musi zostać zainwestowany — a kupowanie „bo są napływy" podnosi kursy niezależnie od fundamentów. Do czasu.'],
        'odplywy_tfi' => ['sentiment', 'NEG', 0, -0.15, 8, 4,
            'Klienci wycofują pieniądze z funduszy akcji',
            'Umorzenia zmuszają fundusze do sprzedawania akcji bez patrzenia na wyceny. Taka podaż potrafi przyginać kursy nawet dobrych spółek.'],
    ];
    ...
    /** Losowanie ważone z katalogu + insert. */
    private static function emitFromCatalog(int $tick, array $catalog, string $scope, ?int $targetId, array $ctx, float $sens): void
    {
        ...
        self::insert($tick, $kind, $type, $scope, $targetId, (int) $espi,
            round((float) $impact * $sens, 3), (int) $dur, strtr($head, $ctx), strtr($body, $ctx));
    }

    public static function insert(int $tick, string $kind, string $type, string $scope, ?int $targetId,
                                  int $espi, float $impact, int $dur, string $head, string $body): void
    {
        Db::pdo()->prepare("INSERT INTO news (headline,body,type,kind,scope,target_id,is_espi,impact_strength,publish_tick,expire_tick,published_at)
                            VALUES (?,?,?,?,?,?,?,?,?,?,?)")
            ->execute([mb_substr($head, 0, 250), $body, $type, $kind, $scope, $targetId, $espi, $impact, $tick, $tick + max(1, $dur), Db::now()]);
    }
