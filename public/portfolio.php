    ...
    <table>
      ...
      <?php foreach ($orders as $o): $isSB = $o['side'] === 'buy' && $o['status'] === 'pending'; $isStop = $o['status'] === 'pending' && !$isSB; $init = (int) ($o['qty_init'] ?? $o['qty']); $rem = (int) $o['qty']; $done = max(0, $init - $rem); ?>
        <tr class="rowlink" onclick="location='order.php?id=<?= (int) $o['id'] ?>'" title="Kliknij — szczegóły zlecenia">
          ...
          <td><?php if ($isSB): ?><span class="chg p" title="Kupno aktywuje się, gdy kurs przebije próg">⏫ STOP-BUY</span>
              <?php elseif ($isStop): ?><span class="chg" style="color:var(--gold);background:var(--gold-bg)">OBRONNE</span>
          ...
          <td class="num"><?php if ($isSB): ?><span class="mono" style="font-size:12px">próg <?= money($o['tp_price']) ?> · limit <?= money($o['price']) ?></span><?php elseif ($isStop): ?><span class="mono" style="font-size:12px"><?= $o['sl_price'] !== null ? (($o['trail_pct'] ?? null) !== null ? 'SL krocz. ' . rtrim(rtrim(number_format((float) $o['trail_pct'], 1, ',', ''), '0'), ',') . '%: ' : 'SL ') . money($o['sl_price']) : '' ?><?= $o['sl_price'] !== null && $o['tp_price'] !== null ? ' · ' : '' ?><?= $o['tp_price'] !== null ? 'TP ' . money($o['tp_price']) : '' ?></span><?php else: ?><?= money($o['price']) ?><?php endif; ?></td>
          <td class="muted hide-m"><?= $isSB ? 'do przebicia progu' : ($isStop ? 'do wyzwolenia' : ($o['expires_session'] !== null ? 'sesja #' . (int) $o['expires_session'] : 'bezterm.')) ?></td>
