...
<?php $ipoOpen = Engine::row("SELECT id, name, ticker, price, close_session FROM ipo_offers WHERE status='open' ORDER BY id DESC LIMIT 1");
      if ($ipoOpen && $sessionNo < (int) $ipoOpen['close_session']): ?>
<a href="ipo.php" class="panel" style="display:block;margin-bottom:16px;border-left:3px solid var(--accent)">
  <b style="font-size:14.5px">📢 Zapisy na IPO: <?= h($ipoOpen['name']) ?> (<?= h($ipoOpen['ticker']) ?>) po <?= money($ipoOpen['price']) ?> PLN</b>
  <span class="muted" style="font-size:12px;display:block;margin-top:2px">Okno zamyka się z końcem sesji #<?= (int) $ipoOpen['close_session'] - 1 ?> · szczegóły i zapis w zakładce IPO →</span>
</a>
<?php endif; ?>
...
<div class="panel" style="margin-bottom:16px;padding:14px 16px 10px">
  <div style="display:flex;align-items:baseline;gap:14px;flex-wrap:wrap">
    <h2 style="margin:0">Indeks MAK40</h2>
    ...
    <span class="muted" style="font-size:12px">ważony kapitalizacją · baza 1000 pkt · <a href="portfolio.php?tab=lok">fundusz indeksowy MAK40 w Portfelu</a></span>
  </div>
...
<div class="panel" style="padding:0;overflow:hidden">
  <div class="tbl-scroll">
    <table class="mw">
      <thead><tr><th style="width:30px"><span title="Obserwowane: gwiazdka przypina spółkę na górze i (z Pakietem Analityka) daje alerty 🔔 przy mocnym sygnale AT">★</span></th><th>Instrument</th><th></th><th class="num">Kurs<?= term('kurs') ?></th><th class="num">Zmiana<?= term('zmiana') ?></th><th class="num hide-m">Sygnał AT<?= $isPremium ? '' : ' 🔒' ?><?= tip('Zbiorczy sygnał analizy technicznej (10 wskaźników z zakładki Analiza). Skaner całego rynku to funkcja Pakietu Analityka — pojedynczo sprawdzisz za darmo na karcie spółki.', '') ?></th><th class="num hide-m">Bid<?= term('bid') ?></th><th class="num hide-m">Ask<?= term('ask') ?></th><th class="num hide-m">Obrót (sesja)<?= tip('Za ile PLN handlowano akcjami tej spółki od otwarcia sesji. Kropka pokazuje płynność: przy niskiej (czerwonej) kupno/sprzedaż większych pakietów rusza kursem.', 'plynnosc') ?></th></tr></thead>
      <tbody>
      <?php foreach ($stocks as $s): $id = (int) $s['id'];
        ...
        <tr onclick="location='stock.php?id=<?= $id ?>'">
          ...
          <td class="num bid hide-m"><?= isset($bid[$id]) ? money($bid[$id]) : '—' ?></td>
          <td class="num ask hide-m"><?= isset($ask[$id]) ? money($ask[$id]) : '—' ?></td>
          <td class="num nowrap hide-m"><span class="mono" data-vol="<?= $id ?>"><?= money_short((float) $s['turnover']) ?></span> <span class="liq <?= $liqCls ?>" title="<?= $liqTxt ?>">●</span></td>
        </tr>
      <?php endforeach; ?>
      </tbody>
    </table>
